      ...
      default:
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', height: '100%' }}>
            <strong>{t('tool.snippets.name')}</strong>
            <span className="c-muted">{ready ? t('tool.snippets.widget.empty') : '…'}</span>
          </div>
        );
    }
  }
  ...
  async function registerCommands(context: ToolContext): Promise<void> {
    context.commands.register({
      ...
      async run(): Promise<CommandResult> {
        ...
        return { ok: true, data: { contextText: t('tool.snippets.widget.empty') } };
      },
    });
  }
