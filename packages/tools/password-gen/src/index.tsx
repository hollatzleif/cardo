...
export function createTool(): CardoTool {
      ...
      default:
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', height: '100%' }}>
            <strong>{t('tool.password-gen.name')}</strong>
            <span className="c-muted">{ready ? t('tool.password-gen.widget.empty') : '…'}</span>
          </div>
        );
    }
  }
  ...
  async function registerCommands(context: ToolContext): Promise<void> {
    context.commands.register({
      id: 'password-gen.context',
      titleKey: 'tool.password-gen.command.context',
      palette: false,
      params: z.object({}),
      selfTestParams: {},
      async run(): Promise<CommandResult> {
        // Summarize the tool's current state for the assistant prompt.
        return { ok: true, data: { contextText: t('tool.password-gen.widget.empty') } };
      },
    });
  }
