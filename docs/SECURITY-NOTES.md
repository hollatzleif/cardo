...
| **Website-Admin-Token** | Nach dem Login im `sessionStorage` – **nur für diesen Tab, nur diese Session**. Die Admin-Seite lädt **bewusst kein Animations-Bundle**: `Base.astro` rendert das Motion-Skript nur bei `motion={true}`, und `admin.astro` setzt `motion={false}`. Auf dieser Seite läuft damit weiterhin **ausschließlich eigener Code** (ein 2-KB-Skript), zusätzlich `meta http-equiv="Content-Security-Policy"` in `Base.astro`. | Ein XSS auf der eigenen statischen Seite (durch fehlenden Fremdcode auf genau dieser Seite praktisch ausgeschlossen) oder physischen Zugriff auf den offenen Tab. |
| **Website: Animationsbibliotheken** | Die Landing Page bündelt GSAP, ScrollTrigger und Lenis (49 KB gzip) als **npm-Abhängigkeiten**, die Vite mitbaut und **gleichursprünglich** ausliefert – kein CDN, kein Laufzeit-Fremdabruf, konform zu `script-src 'self'`. Keine der Bibliotheken öffnet Netzverbindungen; die Aussagen „kein Tracking, keine Analyse, keine Cookies“ bleiben unberührt. Die interaktiven Widgets (Assistent, Karteikarte, Sync, Selbsttest, Themes) sind **eigener Code mit fest hinterlegten Antworten** – kein Modell, keine Anfrage, keine Speicherung außer der Theme-Wahl in `localStorage`. | Die Vertrauensgrenze verschiebt sich von „kein Fremdcode“ zu „Lieferkette der gepinnten npm-Pakete“. Absicherung: `pnpm-lock.yaml` mit Integrity-Hashes und `--frozen-lockfile` in der CI. |

## Bekannte, bewusst akzeptierte Grenzen

- **`vanta` und `three` stehen noch in `apps/website/package.json`, werden aber
  von keiner Seite mehr importiert.** Der WebGL-Hintergrund ist mit dem Umbau
  auf die App-Optik entfallen — ein Cardo-Board hat eine flache Fläche, keinen
  Nebel. Sie sind nicht im Bundle (geprüft: kein `THREE`/`vanta` in
  `dist/_astro/*.js`), sollten aber entweder wieder benutzt oder entfernt
  werden. `vanta@0.5.24` wird seit September 2022 nicht mehr gepflegt und
  zwingt `three` auf exakt `0.134.0` (November 2021).
